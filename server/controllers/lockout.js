...
exports.removeLockOut = function(email, callback) {
    User.update({ email: email }, { $set: { "lockOut.lockedOut" : false } }, function(err) {
        if (err) {
            return callback(err);
        }
...
exports.failedLogIn = function(ip, user, callback) {
    ...
    if (lockObj.fails.length > 5) {
        let validFails = lockObj.fails.filter(function(fail) {
    ...
    // update to db
    User.update({ email: user.email }, { $set: { "lockOut" : lockObj } }, function(err) {
        if (err) {
            return callback(err);
        }
        // if the account has been locked out, email the user to let them know
        if (lockObj.lockedOut) {
            email.lockedOutEmail(user.email, function(err) {
                if (err) {
                    throw(err);
                }
            });
